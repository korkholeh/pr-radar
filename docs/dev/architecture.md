...
| `github_sync` | The httpx GraphQL/REST client, pagination, per-connection rate budgeting, retries, `SyncRun`, `SyncLock`, the orchestration and the post-processing hook | `client.py`, `queries.py`, `rate_limit.py`, `mappers.py`, `upserts.py`, `pipeline.py`, `services.py` |
...
| `ai_detection` | `DetectionRule`, `AISignal`, eight detectors, the PR-template disclosure parser, `ai_status` resolution | `detectors.py`, `disclosure.py`, `rules.py`, `services.py` |
...
| `churn` | Bare git clones, `GIT_ASKPASS` credential handoff, blame-based survival analysis, `ChurnResult` | `clones.py`, `gitcmd.py`, `askpass.py`, `blame.py`, `services.py` |
...
| Alpine.js in the front-end stack | Not used — htmx only, plus three hand-written scripts (`theme.js`, `charts.js`, `formatting.js`) | Nothing in the built UI needed it; adding an unused dependency to the page weight would have been cost without benefit |
