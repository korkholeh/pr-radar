...
| `ai_detection` | `DetectionRule`, `SignalRule`, `AISignal`, twelve detectors, six per-PR and four author-baseline structural kinds, the PR-template disclosure parser, `ai_status` resolution | `detectors.py`, `structural.py`, `baselines.py`, `evidence.py`, `disclosure.py`, `rules.py`, `services.py`, `tasks.py` |
