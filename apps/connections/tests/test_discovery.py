...
from apps.connections.tests.rest_mocks import (
...
def _repo_node(
    ...
    *,
    ...
    """One REST repository object. Discovery reshapes it into the node dict the view consumes."""
    return repo_payload(node_id, name, owner=owner, private=is_private, archived=is_archived)


def _mock_repo_pages(*pages: list[dict]) -> respx.Route:
    """One `/user/repos` page per argument. The organization sweep answers empty here: these tests
    are about the account-level listing, and apps/github_sync/tests/test_discovery.py covers the
    organization fallback on its own."""
    return mock_repository_listing(*pages)
...
def test_discovery_groups_by_owner_hides_archived_and_marks_bound_elsewhere(client, admin_user):
    ...
    page1 = [_repo_node("R_widget", "widget")]
    page2 = [_repo_node("R_gadget", "gadget"), _repo_node("R_archived", "archived-tool", is_archived=True)]
...
def test_owner_filter_narrows_the_list_and_keeps_the_other_owners_selectable(client, admin_user):
    connection = _connection(owner_login="acme")
    page = [_repo_node("R_widget", "widget"), _repo_node("R_portal", "client-portal", owner="client-co")]
    _mock_repo_pages(page)
    client.force_login(admin_user)
...
def test_unknown_owner_filter_falls_back_to_every_owner(client, admin_user):
    connection = _connection(owner_login="acme")
    page = [_repo_node("R_widget", "widget")]
    _mock_repo_pages(page)
    client.force_login(admin_user)
...
def test_show_archived_reveals_archived_repositories(client, admin_user):
    connection = _connection(owner_login="acme")
    page = [_repo_node("R_archived", "archived-tool", is_archived=True)]
    _mock_repo_pages(page)
    client.force_login(admin_user)
...
def test_bulk_add_does_not_silently_rebind_a_repository_bound_elsewhere(client, admin_user):
    ...
    page = [_repo_node("R_gadget", "gadget")]
    _mock_repo_pages(page)
    client.force_login(admin_user)
...
def test_rebind_with_recoverable_github_error_renders_visible_error_not_500(client, admin_user):
    ...
    mock_repository_listing_error(httpx.Response(500, json={"message": "Internal Server Error"}))
    client.force_login(admin_user)
